...
function getCurrentTimeUnit() {
    const selectedValue = document.getElementById('timeUnit').value;
    return timeUnitData[selectedValue];
}

function updateDynamicText() {
    const unit = getCurrentTimeUnit();
    // Update labels in the simulator
    document.getElementById('simDaysLabel').textContent = `${unit.plural} to Simulate:`;
    document.getElementById('maxSeverityLabel').textContent = `Max Severity per ${unit.singular}:`;
    document.querySelector('label[for="duration"]').textContent = `Total Duration (${unit.plural.toLowerCase()}):`;
    document.getElementById('perfectUnitProbLabel').innerHTML = `Perfect ${unit.singular} Probability (%):`;
    document.getElementById('maxEventsPerUnitLabel').innerHTML = `Max Events Per ${unit.singular} (for this analysis):`;
    // Update text in the "How It Works" tab
    document.querySelectorAll('.js-time-unit-plural-lc').forEach(el => el.textContent = unit.plural.toLowerCase());
    document.querySelectorAll('.js-time-unit-singular-lc').forEach(el => el.textContent = unit.singular.toLowerCase());
    document.querySelectorAll('.js-time-unit-plural-cc').forEach(el => el.textContent = unit.plural);
    document.querySelectorAll('.js-time-unit-singular-cc').forEach(el => el.textContent = unit.singular);
    document.getElementById('unit-performance-calc-heading').innerHTML = `🔢 Performance Calculation per ${unit.singular}`;
...
async function runSimulation() {
    try {
        // Get parameters
        const badEvents = parseFloat(document.getElementById('badEvents').value) ||
            0;
        const duration = parseFloat(document.getElementById('duration').value) || 1;
        ...
        const targetEfficiency = parseFloat(document.getElementById('targetEfficiency').value) / 100 || 0.85;
        ...
        const timeUnit = getCurrentTimeUnit();
...
`;
        ...
        for (let day = 0; day < simDays; day++) {
            ...
            const eventsDisplay = eventsToday.length > 0 ? eventsToday.join(', ') : `None (Perfect ${timeUnit.singular}! 🌟)`;
            const forecastText = forecast === 0 ?
                ...
                `Need ${forecast} perfect ${timeUnit.plural.toLowerCase()} to reach ${(targetEfficiency * 100).toFixed(2)}% goal (EWMA)`;
            dayDiv.innerHTML = `
<div><strong>📅 ${timeUnit.singular} ${report.day}</strong></div>
<div>Events: ${eventsDisplay}</div>
<div>Performance per ${timeUnit.singular}: ${(report.dailyPerf * 100).toFixed(2)}%</div>
...
<div>Goal ${goalAchieved ? `achieved on ${timeUnit.singular.toLowerCase()} ${goalDay}` : 'not achieved'}</div>
<div>═══════════════════════════════════════</div>
`;
...
<div class="stat-label">Perfect ${timeUnit.plural}</div>
</div>
<div class="stat-card">
<div class="stat-value">${imperfectDays}</div>
<div class="stat-label">Imperfect ${timeUnit.plural}</div>
</div>
<div class="stat-card">
<div class="stat-value">${perfectDayRate}%</div>
<div class="stat-label">Perfect ${timeUnit.singular} Rate</div>
</div>
<div class="stat-card">
<div class="stat-value">${imperfectDayRate}%</div>
<div class="stat-label">Imperfect ${timeUnit.singular} Rate</div>
</div>
<div class="stat-card">
<div class="stat-value">${simDays}</div>
<div class="stat-label">Simulation ${timeUnit.plural}</div>
</div>
<div class="stat-card">
...
async function runMultipleSimulations() {
    try {
        ...
        const useProbabilities = document.getElementById('useProbabilitiesInMultiSim').value === 'yes';
        ...
        // --- Calculate "Actual Perfect Days Needed" ---
        ...
        for (let simId = 0; simId < nRuns; simId++) {
            ...
            for (let day = 0; day < simDays; day++) {
                let eventsToday;
                if (useProbabilities) {
                    eventsToday = getRandomEvents(severityProfile, perfectDayProb, secondEventProb);
                } else {
                    eventsToday = getRandomEventsForMultipleSims(eventPool, maxEventsPerDay);
                }
        ...
        displayMultiSimStats(runData, initialEfficiency, targetEfficiency, perfectDaysNeeded, timeUnit);
        renderCharts(runData, timeUnit);
    } catch (error) {
...
function displayMultiSimStats(runData, initialEfficiency, targetEfficiency, perfectDaysNeeded, timeUnit) {
...
<div class="stat-label">Avg.
Final Efficiency</div>
</div>
<div class="stat-card">
...
<div class="stat-label">Avg.
${timeUnit.plural} to Goal (EWMA)</div>
</div>
<div class="stat-card">
<div class="stat-value">${maxDaysToGoal}</div>
<div class="stat-label">Max ${timeUnit.plural} to Goal (EWMA)</div>
</div>
<div class="stat-card">
...
<div class="stat-card">
<div class="stat-value">${isFinite(perfectDaysNeeded) ?
(perfectDaysNeeded < 0 ? 0 : perfectDaysNeeded) :  '∞' }</div>
<div class="stat-label">Actual Perfect ${timeUnit.plural} (Simple Model)</div>
</div>
`;
    statsContainer.style.display = 'grid';
}

function renderCharts(runData, timeUnit) {
    ...
    myCharts.push(new Chart(effCtx, {
        ...
        options: {
            plugins: {
                title: {
                    display: true,
                    text: 'Final Efficiency Distribution (%)'
                }
            },
            scales: {
                x: {
    ...
    myCharts.push(new Chart(goalCtx, {
        ...
        options: {
            plugins: {
                title: {
                    display: true,
                    text: `${timeUnit.plural} to Reach Goal Distribution`
                }
            },
            scales: {
                x: {
                    title: {
                        display: true,
                        text: `${timeUnit.plural} to Goal Bins`
                    }
                },
                y: {
                    title: {
    ...
    const lineLabels = useMovingAverage ?
        Array.from({
            length: efficiencyLineData.length
        }, (_, i) => `Run ${i * windowSize + 1}-${(i + 1) * windowSize}`) :
    ...
    myCharts.push(new Chart(effLineCtx, {
        ...
        options: {
            plugins: {
                title: {
                    display: true,
                    text: `Final Efficiency Over Simulations${lineChartTitleSuffix}`
                }
            },
            scales: {
                x: {
                    title: {
                        display: true,
                        text: useMovingAverage ?
                            'Simulation Window' : 'Simulation ID'
                    }
                },
                y: {
                    title: {
                        display: true,
                        text: 'Final Efficiency (%)'
                    }
                }
            }
        }
    ...
    myCharts.push(new Chart(daysLineCtx, {
        ...
        data: {
            labels: lineLabels,
            datasets: [{
                label: `${timeUnit.plural} to Reach Goal`,
                data: daysLineData,
        ...
        options: {
            plugins: {
                title: {
                    display: true,
                    text: `${timeUnit.plural} to Goal Over Simulations${lineChartTitleSuffix}`
                }
            },
            scales: {
                x: {
                ...
                y: {
                    title: {
                        display: true,
                        text: `${timeUnit.plural} to Reach Goal`
                    }
                }
            }
        }
...
function createHistogramData(data, numBins) {
    ...
    data.forEach(value => {
        ...
        // Increment the 
        // count for the corresponding bin
...
function toggleProbWarning() {
  const select = document.getElementById('useProbabilitiesInMultiSim');
  const warning = document.getElementById('probWarning');
  warning.style.display = (select.value === 'yes') ? 'block' : 'none';
}
...
document.getElementById('timeUnit').addEventListener('change', updateDynamicText);


// Initialize the page
updateInitialEfficiency();
updateDynamicText();
toggleProbWarning(); // ensures it reflects the initial selected value
